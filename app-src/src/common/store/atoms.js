...
export const servicesAtom = atomWithReducer(fetchServices(), () =>
  fetchServices(),
)
...
export const tasksAtomNew = atom(async (get) => {
  // Reload when view changed
  get(viewAtom)
  return await fetch(baseUrl + 'ui/tasks').then((data) => data.json())
})
...
export const logsServicesAtom = atom(async (get) => {
  // Reload when view changed
  get(viewAtom)
  return await fetch(baseUrl + 'ui/logs/services').then((data) => data.json())
})
export const serviceDetailAtom = atom(async (get) => {
  let id = get(viewAtom)['detail']
  return await fetch(baseUrl + 'docker/services/' + id).then((data) =>
    data.json(),
  )
})
