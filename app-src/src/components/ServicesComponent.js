            ...
            trows.push(
                <tr key={'tr' + node['ID']} className={node['Status']['State'] === 'ready' ? null : 'danger'}>
                    <td className='align-middle'>
                        <Link to={'/nodes/' + node['ID']} ><Button variant="secondary" size="sm" className='w-100 text-nowrap' onClick={() => this.showNodeDetails()}>{node['Description']['Hostname']}  {node['ManagerStatus'] && node['ManagerStatus']['Leader'] && <FontAwesomeIcon icon='star' />}</Button></Link>
                    </td>
