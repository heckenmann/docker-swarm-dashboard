...
describe('detail atoms guards and fetch', () => {
  ...
  test('serviceDetailAtom fetches when view.id matches and returns json', async () => {
    ...
    const atoms = require('../../../src/common/store/atoms')
    const get = (a) =>
      a === atoms.viewAtom ? { id: 'servicesDetail', detail: 's1' } : '/'
  ...
  test('nodeDetailAtom fetches when view.id matches and returns json', async () => {
    ...
    jest.doMock('jotai-location', () => ({ atomWithHash: (k, def) => def }))
    global.fetch = jest.fn(() =>
      Promise.resolve({ json: () => Promise.resolve({ node: true }) }),
    )
    const atoms = require('../../../src/common/store/atoms')
    const nav = require('../../../src/common/navigationConstants')
    const get = (a) =>
      a === atoms.viewAtom ? { id: nav.nodesDetailId, detail: 'n1' } : '/'
    const res = await atoms.nodeDetailAtom(get)
  ...
  test('exhaustive detail atoms branches (mismatch, missing id, and fetch)', async () => {
    ...
    res = await doFetched(atoms.nodeDetailAtom, {
      id: nav.nodesDetailId,
      detail: 'nX',
    })
    expect(res).toEqual({ ok: true })
    ...
    expect(res).toBeNull()
    res = await doFetched(atoms.serviceDetailAtom, {
      id: nav.servicesDetailId,
      detail: 'sY',
    })
    expect(res).toEqual({ ok: true })
    ...
    expect(res).toBeNull()
    res = await doFetched(atoms.taskDetailAtom, {
      id: nav.tasksId,
      detail: 'tZ',
    })
    expect(res).toEqual({ ok: true })
  ...
  test('taskDetailAtom fetches when view.id matches and returns json', async () => {
    ...
    jest.doMock('jotai-location', () => ({ atomWithHash: (k, def) => def }))
    global.fetch = jest.fn(() =>
      Promise.resolve({ json: () => Promise.resolve({ task: true }) }),
    )
    const atoms = require('../../../src/common/store/atoms')
    const nav = require('../../../src/common/navigationConstants')
    const get = (a) =>
      a === atoms.viewAtom ? { id: nav.tasksId, detail: 't1' } : '/'
    const res = await atoms.taskDetailAtom(get)
  ...
  test('serviceDetailAtom fetches when view.id matches and returns json', async () => {
    ...
    jest.doMock('jotai-location', () => ({ atomWithHash: (k, def) => def }))
    global.fetch = jest.fn(() =>
      Promise.resolve({ json: () => Promise.resolve({ svc: true }) }),
    )
    const atoms = require('../../../src/common/store/atoms')
    const nav = require('../../../src/common/navigationConstants')
    const get = (a) =>
      a === atoms.viewAtom ? { id: nav.servicesDetailId, detail: 'sX' } : '/'
    const res = await atoms.serviceDetailAtom(get)
